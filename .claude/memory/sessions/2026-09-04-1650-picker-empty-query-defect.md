...
- **BOTH PROOF CASES SUBSEQUENTLY TESTED AND CONFIRMED by the user (2026-09-04), after the
  merge:** opening a picker without typing, and clearing a set sales owner then saving.
  So this fix is verified behaviourally, not just accepted by a paste — the earlier caveat
  on this line is discharged, not merely softened.
- Merged as PR #56 (`7b20956`). PR #55's merge commit was the "main ahead by 1" — a merge
  commit whose tree is identical to the branch tip it merged, so it carried no content.
