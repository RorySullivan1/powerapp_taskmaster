...
- **scrHome's three charts are REAL SVG now, and UNPROVEN in Studio** — a Gantt of the
  projects you lead plus two donuts with in-chart legends. First use in this repo of
  `Concat`, `Sequence`+`Index`, a self-joining running total and `Text(...,"en-US")`.
  If one fails to paste, the three are independent controls, so the failure localises.
