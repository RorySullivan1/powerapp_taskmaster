...
> ## READ THIS FIRST — where this document is WRONG for this repo
>
> This is Microsoft's generic guidance, kept verbatim as a structural reference. It is
> accurate about the pa-yaml **shape** and about Power Fx **semantics**. It is *not* accurate
> about the control tokens Studio actually emits, and following it literally produces failed
> pastes. The four `powerapp-canvas-*` skills carry the corrections; the headlines:
>
> | This document says | Reality in this dialect |
> |---|---|
> | `Control: Button`, `TextInput`, `Container`, `Dropdown` | `Classic/Button@2.2.0`, `ModernTextInput@1.0.0`, `GroupContainer@1.5.0` + `Variant: AutoLayout`, `ModernCombobox@1.0.0` |
> | `Control: Component` for a component instance | `Control: CanvasComponent` |
> | "Comments not supported in YAML context" | Power Fx `//` comments work inside block scalars, and this repo relies on them |
> | Gallery templates nested under a `GalleryTemplate` child | children sit directly under the gallery's `Children:` |
> | `IfError(Patch(…), Notify(…))` as the error idiom | rejected today — every `IfError` argument must be type-compatible |
> | Column names as strings — `ShowColumns(t, "Name")` | identifiers since 3.24042 — `ShowColumns(t, Name)` |
> | Layout formulas are how you get responsive design | true in Studio, but X/Y/Width/Height **freeze to constants on paste** |
>
> Nothing here is edited, so it can be diffed against future versions upstream.
